...
- (void)fe_viewWillAppear:(BOOL)animated{
    NSLog(@"ViewController-fe_viewWillAppear--实现");
}
...
- (void)setup{
//    CardCourse *cardCourse = [CardCourse new];
//    [cardCourse testCourse];
    
    NSString *arrCls = NSStringFromClass([NSArray class]);
//    NSLog(@"NSArray----%@",[NSArray class]);
    NSArray *arr = @[@"11"];
    NSLog(@"%@", [NSArray class]);
    NSLog(@"%@", [arr class]);
